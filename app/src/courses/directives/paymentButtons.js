(function(module) {
    ...
    module.controller('paymentButtons', function($state, $rootScope, TakeCourse, $location, $uibModal, GetCourseUserEntry, $scope, UserSubscription, InstructionLevelSubscription, flash, $filter) {
        var model = this;
                    ...
                    model.takeCourse.$save()
                        .then(function(response) {
                            if (response.id && response.id !== null) {
                                ...
                                $location.path("/learn-course/" + courseID + "/" +  courseTitle);
                            } else {
