...
} (angular.module('ace.offlineCourse', [
    'ui.router',
    'ngResource',
])));

(function (module) {
    module.directive('offlineCourseBatch', function () {
        var linker = function (scope, element, attrs) {
            // do DOM Manipulation here
        };
        return {
            restrict: 'AE',
            templateUrl: 'src/plugins/OfflineCourse/offlineCourseBatch.tpl.html',
            link: linker,
            controller: 'offlineCourseBatchController as model',
    ...
    module.controller('offlineCourseBatchController', function ($rootScope, $scope, OfflineBatchesLists, $state, $uibModal, $uibModalStack, $filter, DemoSessionList, ConstTimeZoneConversion) {
        /*Constants*/
        ...
        if ($state.params.id !== null && $state.params.id !== undefined) {
            ...
            OfflineBatchesLists.get(params).$promise
                    ...
                    if (angular.isDefined(response.data)) {
                        ...
                        angular.forEach(response.data, function (batch) {
                            if (batch.is_offline === true) {
                                ...
                                if (batch.course_offline_sessions !== null) {
                                    batch.notifications = {};
                                    batch.course_offline_sessions = $filter('CountryTimezone')(batch.course_offline_sessions, ['session_date','session_end_date'], 'TimeZoneSessionSet', 'dd MMM yyyy hh:mm:ss');
                                    angular.forEach(batch.course_offline_sessions, function (session) {
                                        ...
                                        if (!angular.isDefined(batch.notifications[session_month])) {
                                            ...
                                            if (angular.isDefined(batch.notifications[session_month])) {
                                                batch.notifications[session_month][0].days.push({
                                                    ...
                                                    'time': session_time,
                                                    'end_time': $filter('date')(Date.parse(session.session_end_date), 'hh:mm a'),
                                    ...
                                    model.offline_batch_listing.push(batch);
                                }
                            }
        ...
        $scope.modalDemoShow = function (e) {
            e.preventDefault();
            $scope.modalInstance = $uibModal.open({
                scope: $scope,
                size: 'lg',
                templateUrl: 'src/plugins/OfflineCourse/offlineDemoBatch_model.tpl.html',
                resolve: {
                    pageType: function () {
                        return "modal";
                    },
        ...
        $scope.modalOfflineBatch = function (e, index) {
            e.preventDefault();
            $scope.modalInstance = $uibModal.open({
                scope: $scope,
                size: 'lg',
                templateUrl: 'src/plugins/OfflineCourse/offlineBatch_model.tpl.html',
                resolve: {
                    pageType: function () {
                        return "modal";
                    },
    ...
    module.directive('offlineCourse', function () {
        var linker = function (scope, element, attrs) {
            // do DOM Manipulation here
        };
        return {
            restrict: 'AE',
            templateUrl: 'src/plugins/OfflineCourse/offlineCourseButton.tpl.html',
            link: linker,
            controller: 'OfflineCourseButtonController as model',
            bindToController: true,
            scope: {
                courseId: '@courseId'
            }
    ...
    module.controller('OfflineCourseButtonController', function ($rootScope) {
        var model = this;
    });
    module.controller('offlineCourseController', function ($state, CourseOfflineSession, TimezoneList, $location, $scope, flash, $filter, $rootScope, $interval, User, UserProfile, OfflineBatchesLists, $uibModal, $uibModalStack, $timeout, GetCurrencyList, OfflineCourseService, AlertBox, ConstDateFormat, ConstToolTipContent) {
        $rootScope.pageTitle = $filter("translate")("Manage Led Live Onsite Training")+ " | " +$rootScope.settings['site.name'];
        ...
        $scope.offline_course.course_id = $state.params.id;
        /**FUNCTION ASSIGNING WITH MODEL */
        model.addCourseOfflineSession = addCourseOfflineSession;
        model.editCourseOfflineSession = editCourseOfflineSession;
        $scope.ConstDateFormat = ConstDateFormat;
        model.removeCourseOfflineSession = removeCourseOfflineSession;
        model.paginate = paginate;
        model.sort = sort;
        model.CurrencyTireFilter = CurrencyTireFilter;
        model.TireFilter = TireFilter;
        model.AddingOfflineSession = AddingOfflineSession;
        model.dateInit = dateInit;
        model.sessions = [];
        model.offline_action = 'list';
        ...
        function dateInit() {
            ...
            angular.forEach(model.currencies, function (currency) {
                if ($rootScope.settings['site.currency_code'] === currency.code) {
                    ...
                    $scope.offline_course.currency_id = currency.id;
        ...
        $scope.init = function (element) {
            ...
            if (model.currentSortby !== undefined && model.currentSortby !== null) {
                params.filter = model.currentSortby;
            } else {
                params.filter = 'all';
                    ...
                    if (response.data !== undefined && response.data.length > 0) {
                        ...
                        angular.forEach(response.data, function (value) {
                            if (value.is_offline) {
                                ...
                                model.offlineCoursesList.push(value);
                            }
                        });
                    }
        ...
        var unwatchMinMaxValues = $scope.$watch(function () {
            ...
            // min max dates
            /*Offline Course Add date updating */
            ...
            if (model.picker1.date !== null && model.picker2.date !== null && model.picker1.date !== undefined && model.picker2.date !== undefined) {
                $scope.offline_course.start_date = $filter('date')(Date.parse(model.picker1.date), ConstDateFormat.created);
                $scope.offline_course.end_date = $filter('date')(Date.parse(model.picker2.date), ConstDateFormat.created);
                ...
                if (model.sessions.length > 0) {
                    angular.forEach(model.sessions, function (session) {
                        if ($filter('date')(Date.parse(session.session_date), 'yyyy-MM-dd') < $filter('date')(Date.parse($scope.session_datepickerOptions.minDate), 'yyyy-MM-dd') || $filter('date')(Date.parse(session.session_date), 'yyyy-MM-dd') > $filter('date')(Date.parse($scope.session_datepickerOptions.maxDate), 'yyyy-MM-dd')) {
            ...
            if (model.edit_picker1 !== null && model.edit_picker2 !== null && model.edit_picker1 !== undefined && model.edit_picker2 !== undefined) {
                $scope.edit_offline_course.start_date = $filter('date')(Date.parse(model.edit_picker1.date), ConstDateFormat.created);
                $scope.edit_offline_course.end_date = $filter('date')(Date.parse(model.edit_picker2.date), ConstDateFormat.created);
        ...
        // google autocomplete address formatting function  ..
        $scope.$on('g-places-autocomplete:select', function (event) {
            angular.forEach(event.targetScope.model.address_components, function (value) {
                if (value.types[0] === 'postal_code') {
                    $scope.offline_course.zip_code = value.short_name;
                    if ($scope.edit_offline_course !== null && $scope.edit_offline_course !== undefined) {
                        if (model.editform === true) {
                            $scope.edit_offline_course.zip_code = value.short_name;
                        }
                    }
                }
            });
        ...
        function removeCourseOfflineSession(index, scope_variable) {
        ...
        //Adding the session
        function AddingOfflineSession(type, scope_variable) {
        ...
        function addCourseOfflineSession($valid, Form) {
            if ($valid) {
                if (model.sessions.length === 0) {
                    flashMessage = $filter("translate")("Sorry! You must have atleast one session to create a  Instructor-led live onsite training classes.");
                    flash.set(flashMessage, 'info', false);
                ...
                if (typeof (model.place) !== 'string' && model.place !== null && model.place.address_components !== undefined) {
                    $scope.offline_course.address = model.place.formatted_address;
                    $scope.offline_course.latitude = model.place.geometry.location.lat();
                    $scope.offline_course.longitude = model.place.geometry.location.lng();
                    angular.forEach(model.place.address_components, function (value) {
                        if (value.types[0] === 'locality' || value.types[0] === 'administrative_area_level_2') {
                            $scope.offline_course.city_name = value.long_name;
                        }
                        if (value.types[0] === 'administrative_area_level_1') {
                            $scope.offline_course.state_name = value.long_name;
                        }
                        if (value.types[0] === 'country') {
                            $scope.offline_course.country_iso2 = value.short_name;
                        }
                    });
                    model.start_placeInvalid = false;
                } else {
                    $scope.offline_course.address = model.place;

                }
                // $scope.offline_disableButton = true;
                $scope.offline_course.sessions = [];
                var timezone, chosedtimezone;
                angular.forEach(model.sessions, function (session, key) {
                    ...
                    $scope.offline_course.sessions.push({
                ...
                CourseOfflineSession.create($scope.offline_course, function (response) {
                    if (response.error.code === 0) {
                        UpdateCourseStatus();
                        flashMessage = $filter("translate")("Instructor-led live onsite training classes has been Added successfully.");
                        ...
                        flashMessage = $filter("translate")("Instructor-led live onsite training classes couldn't be Added.Try again later");
                        flash.set(flashMessage, 'error', false);
                    }
                    $scope.offline_disableButton = false;
                }, function (error) {
                    flashMessage = $filter("translate")("Error occurred while adding Instructor-led live onsite training classes .Try again later");
                    flash.set(flashMessage, 'error', false);
                    $scope.offline_disableButton = false;
                });
            }
        ...
        $scope.EditOfflineCourse = function (id, index) {
            $scope.edit_offline_course = {};
            OfflineCourseService.get({
                id: id
            }).$promise
                .then(function (response) {
                    if (response.data.length > 0) {
                        if (response.data[0].course_offline_sessions !== null && response.data[0].course_offline_sessions !== undefined) {
                            if (response.data[0].course_offline_sessions.length > 0) {
                                $scope.edit_offline_course.id = response.data[0].id;
                                model.edit_place = response.data[0].course_offline_sessions[0].address;
                                $scope.edit_offline_course.zip_code = response.data[0].course_offline_sessions[0].zip_code;
                                ...
                                angular.forEach(model.currencies, function (currency) {
                                    if (parseInt(response.data[0].course_batche_currency_id) === parseInt(currency.id)) {
                                        $scope.selected = currency;
                                        $scope.edit_offline_course.currency_id = currency.id;
                                        if (currency.currencies_tiers !== null && response.data[0].course_batche_tier_id !== null) {
                                            ...
                                            angular.forEach(currency.currencies_tiers, function (tier) {
                                                if (parseInt(response.data[0].course_batche_tier_id) === parseInt(tier.tier_id)) {
                                                    $scope.tier_selected = tier;
                                                    $scope.edit_offline_course.tier_id = tier.tier_id;
                                                    $scope.edit_offline_course.price = $scope.tier_selected.amount;
                                                }
                                            });
                                        }
                                    }
                                ...
                                response.data[0].course_offline_sessions = $filter('CountryTimezone')(response.data[0].course_offline_sessions, ['session_date', 'session_end_date'], 'TimeZoneSessionSet', ConstDateFormat.medium_created_24);
                                ...
                                /*Pushing the session */
                                angular.forEach(response.data[0].course_offline_sessions, function (offline_session) {
                                    var format = offline_session.session_date;
                                    model.edit_sessions.push({
                                        "session_date": new Date(offline_session.session_date),
                                        "session_time": new Date(offline_session.session_date),
                                        "timezone_id": offline_session.timezone_id,
                                        "lesson_description": offline_session.lesson_description,
                                        "lesson_topic": offline_session.lesson_topic,
                                        "session_end_date": new Date(offline_session.session_end_date),
                                        "open": false,
                                    });
                                });
                        ...
                        model.offline_action = 'edit';
                    }
                });
        };
        // Adding or Creating the offline sessions
        function editCourseOfflineSession($valid, Form) {
            if ($valid) {
                if (model.edit_sessions.length === 0) {
                    flashMessage = $filter("translate")("Sorry! You must have atleast one session to create a  Instructor-led live onsite training classes.");
                    flash.set(flashMessage, 'info', false);
                ...
                if (typeof (model.edit_place) !== 'string' && model.edit_place !== null && model.edit_place.address_components !== undefined) {
                    $scope.edit_offline_course.address = model.edit_place.formatted_address;
                    $scope.edit_offline_course.latitude = model.edit_place.geometry.location.lat();
                    $scope.edit_offline_course.longitude = model.edit_place.geometry.location.lng();
                    angular.forEach(model.edit_place.address_components, function (value) {
                        if (value.types[0] === 'locality' || value.types[0] === 'administrative_area_level_2') {
                            $scope.edit_offline_course.city_name = value.long_name;
                        }
                        if (value.types[0] === 'administrative_area_level_1') {
                            $scope.edit_offline_course.state_name = value.long_name;
                        }
                        if (value.types[0] === 'country') {
                            $scope.edit_offline_course.country_iso2 = value.short_name;
                        }
                    });
                    model.start_placeInvalid = false;
                } else {
                    $scope.edit_offline_course.address = model.edit_place;

                }
                $scope.edit_offline_course.sessions = [];
                var timezone, chosedtimezone;
                angular.forEach(model.edit_sessions, function (session, key) {
                    ...
                    var sessionDate_Time = $filter('date')(Date.parse(session.session_date), ConstDateFormat.created) + ' ' + $filter('date')(Date.parse(session.session_time), ConstDateFormat.time_sec_24);
                    sessionDate_Time = $filter('date')(Date.parse(sessionDate_Time + timezone), ConstDateFormat.created_24_z, '+0000');
                    var sessionEndDate_Time = $filter('date')(Date.parse(session.session_date), ConstDateFormat.created) + ' ' + $filter('date')(Date.parse(session.session_end_date), ConstDateFormat.time_sec_2);
                    sessionEndDate_Time = $filter('date')(Date.parse(sessionEndDate_Time + timezone), ConstDateFormat.created_24_z, '+0000');
                    $scope.edit_offline_course.sessions.push({
                        'session_date': sessionDate_Time,
                        'timezone_id': session.timezone_id,
                        'lesson_description': session.lesson_description,
                        'lesson_topic': session.lesson_topic,
                ...
                $scope.edit_offline_course.course_id = $state.params.id;
                OfflineCourseService.update({
                    id: $scope.edit_offline_course.id,
                }, $scope.edit_offline_course, function (response) {
                    if (response.error.code === 0) {
                        UpdateCourseStatus();
                        $scope.init(null);
                        model.offline_action = 'list';
                        flashMessage = $filter("translate")("Instructor-led live onsite training classes has been updated successfully.");
                        flash.set(flashMessage, 'success', false);
                        $scope.edit_offline_course = {};
                    } else {
                        flashMessage = $filter("translate")("Instructor-led live onsite training classes couldn't be updated.Try again later");
                        flash.set(flashMessage, 'error', false);
                    }
                }, function (error) {
                    flashMessage = $filter("translate")("Error occurred while updating Instructor-led live onsite training classes .Try again later");
                    flash.set(flashMessage, 'error', false);
                    $scope.offline_disableButton = false;
                });
            }
        }
        //Offline course Delete function
        $scope.DeleteOfflineCourse = function (id) {
            AlertBox.confirm('Are you sure you want to delete this Instructor-led live onsite training class?', function (isConfirmed) {
                if (isConfirmed) {
                    OfflineCourseService.remove({
                        id: id,
                    }, function (response) {
                        if (response.error.code === 0) {
                            $scope.init(null);
                            flashMessage = $filter("translate")("Instructor-led live onsite training class has been deleted successfully.");
                            flash.set(flashMessage, 'success', false);
                        } else {
        ...
        $scope.modalBatch = function (e, Batch, model_type) {
            ...
            if (model_type === 'Students') {
                template = 'src/plugins/OfflineCourse/enrolledstudent_model.tpl.html';
            } else {
                template = 'src/plugins/OfflineCourse/session_model.tpl.html';
            }
            if (angular.isUndefined($rootScope.modal) || !$rootScope.modal) {
                $scope.modalInstance = $uibModal.open({
                    scope: $scope,
                    ...
                    controller: function ($scope, Batch, $filter, modelType) {
                        var model = this;
                        $scope.batch_list = Batch;
                        if (modelType !== 'Students') {
                            angular.forEach($scope.batch_list.course_offline_sessions, function (offline_session) {
...
(function (module) {
    ...
    module.factory('CourseOfflineSession', function ($resource, GENERAL_CONFIG) {
        return $resource(
            GENERAL_CONFIG.api_url + 'api/v1/course_offline_sessions', {
    ...
    module.factory('OfflineCourseService', function ($resource, GENERAL_CONFIG) {
        return $resource(
            GENERAL_CONFIG.api_url + 'api/v1/course_offline_batches/:id/course_offline_sessions', {
                id: '@id'
            }, {
                get: {
                    method: 'GET',
    ...
    module.factory('OfflineBatchesLists', function ($resource, GENERAL_CONFIG) {
        return $resource(
