...
(function(module) {
    module.directive('payOut', function() {
        ...
        return {
            restrict: 'A',
            templateUrl: 'src/plugins/ZazPayPayout/payoutButton.tpl.html',
            link: linker,
    ...
    module.controller('payoutController', function($state, PayoutList, PayoutRedirect, $location, $scope, flash, $filter, $rootScope, TokenServiceData, $interval) {
        var model = this;
        model.loading = true;
        $rootScope.pageTitle = $filter("translate")("Manage Payout")+ " | " +$rootScope.settings['site.name'];
        ...
        var connected_gateway_id = $state.params.zazpay_gateway_id;
                ...
                .then(function(response) {
                    ...
                    angular.forEach(model.availPayouts, function(key, value) {
                        if (key.is_connected === true) {
                            current_payout_id = key.zazpay_gateway_id;
                        }
                    });
        ...
        function connectToPayment(zazpayid, event) {
            event.preventDefault();
            model.payment.gateway_id = zazpayid;
